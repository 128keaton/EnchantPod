...
import com.keaton.enchantpod.EnchantPodEntityListener;

/**
 * @author Randy
 *
 */
public class EnchantPod extends JavaPlugin {

	@Override
	public void onEnable() {
		this.saveDefaultConfig();
		
		getLogger().info("EnchantPod has been enabled.");
		getServer().getPluginManager().registerEvents( new EnchantPodEntityListener(this), this );
	}
	
	@Override
	public void onDisable() {
		getLogger().info("EnchantPod has been disabled.");
	}
	
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(cmd.getName().equalsIgnoreCase("enegg")) {
			...
			if( args[0].equalsIgnoreCase("debug")) {
				...
				if( eggDebug ) {
					...
					sender.sendMessage("Debugging debug disabled");
				}
			}
			...
			if( args[0].equalsIgnoreCase("lootrate")) {
				if( args.length < 2) {
					return false;
				}
